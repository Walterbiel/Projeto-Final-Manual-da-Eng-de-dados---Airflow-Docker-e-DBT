...
# Dispara em pushes para main e em Pull Requests
on:
  push:
    branches: [main]
  pull_request:
...
jobs:
  ...
  # ─────────────────────────────────────────────────────
  # Job 1: Compilação — valida sintaxe sem banco real
  # Roda em ~2 min. Feedback rápido para erros de SQL.
  # ─────────────────────────────────────────────────────
  dbt-compile:
  ...
  # Só roda se o Job 1 passar (needs: dbt-compile)
  # ─────────────────────────────────────────────────────
  dbt-build:
    name: Executar pipeline completo (seed + run + test)
    runs-on: ubuntu-latest
    needs: dbt-compile  # Dependência: só executa se a compilação passar

    services:
      postgres:              # GitHub Actions sobe este container como serviço
        image: postgres:17
        env:
        ...
        options: >-
          ...
          --health-timeout 5s
          --health-retries 5
          # O job só começa após o banco estar healthy
    ...
    steps:
      ...
      - name: Testar conexão com o banco (dbt debug)
        working-directory: 2_data_warehouse/dw_bootcamp
        run: ../../1_local_setup/.venv/bin/dbt debug

      - name: Carregar dados brutos (dbt seed)
        working-directory: 2_data_warehouse/dw_bootcamp
        run: ../../1_local_setup/.venv/bin/dbt seed
        # Carrega o CSV de 41 MB no PostgreSQL do CI (~5-10 min)

      - name: Executar models e testes (dbt build)
        working-directory: 2_data_warehouse/dw_bootcamp
        run: ../../1_local_setup/.venv/bin/dbt build --exclude-resource-type seed

      - name: Gerar documentação dbt
        working-directory: 2_data_warehouse/dw_bootcamp
        run: ../../1_local_setup/.venv/bin/dbt docs generate
      ...
      - name: Publicar artefatos de documentação
        ...
        with:
          ...
          retention-days: 7
          # Os docs ficam disponíveis para download por 7 dias na aba Actions
```
